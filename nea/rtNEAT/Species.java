...
public class Species implements Comparable<Species>{
	...
	public boolean reproduce(int generation, Population pop, Vector<Species> sorted_species) {
		int count;
		Organism curorg; // = new Vector<Organism>();
		...
		Species cursp; // = new Vector<Species>();
		...
		for (count = 0; count < expected_offspring; count++) {
			...
			if(thechamp.super_champ_offspring > 0){
				...
				if ((Math.random() < 0.8) || (Neat.mutate_add_link_prob == 0.0)) 
					//ABOVE LINE IS FOR:
					//Make sure no links get added when the system has link adding disabled
					new_genome.mutate_link_weights(mut_power, 1.0, Genome.mutator.GAUSSIAN);
				else {
					//Sometimes we add a link to a superchamp
					net_analogue = new_genome.genesis(generation);
					new_genome.mutate_add_link(pop.innovation, pop.cur_innov_num, Neat.newlink_tries);
					//delete net_analogue;
				...
				new_genome = thechamp.gnome.duplicate(count);

				if (Math.random() < Neat.mutate_random_trait_prob) {
					//std::cout<<"mutate random trait"<<std::endl;
					new_genome.mutate_random_trait();
				}
				if (Math.random()<Neat.mutate_link_trait_prob) {
					//std::cout<<"mutate_link_trait"<<std::endl;
					new_genome.mutate_link_trait(1);
				}
				if (Math.random() < Neat.mutate_node_trait_prob) {
					//std::cout<<"mutate_node_trait"<<std::endl;
					new_genome.mutate_node_trait(1);
				}
				if (Math.random()<Neat.mutate_link_weights_prob) {
					//std::cout<<"mutate_link_weights"<<std::endl;
					new_genome.mutate_link_weights(mut_power, 1.0, Genome.mutator.GAUSSIAN);
				}
				if (Math.random() < Neat.mutate_toggle_enable_prob) {
					//std::cout<<"mutate toggle enable"<<std::endl;
					new_genome.mutate_toggle_enable(1);

				}
				if (Math.random()< Neat.mutate_gene_reenable_prob) {
			...
			baby = new Organism(0.0, new_genome, generation);
				...
				if ((!champ_done)&&
					...
					//If there is only one organism in the pool, then always mutate
				else if ((Math.random()< Neat.mutate_only_prob) || poolsize == 0) {

						//Choose the random parent

						//RANDOM PARENT CHOOSER
						orgnum = (int) (Math.random()*poolsize); // randint(0, poolsize);
						//curorg = organisms.begin();
						curorg = organisms.get(0);
						for(orgcount=0; orgcount < orgnum; orgcount++)
							//++curorg;     
							curorg = organisms.get(orgcount);
						...
						new_genome = curorg.gnome.duplicate(count); //mom.gnome.duplicate(count);

						//Do the mutation depending on probabilities of 
						//various mutations

						if (Math.random() < Neat.mutate_add_node_prob) {
							//std::cout<<"mutate add node"<<std::endl;
							new_genome.mutate_add_node(pop.innovations, pop.cur_node_id, pop.cur_innov_num);
							mut_struct_baby = true;
						}
						else if (Math.random() < Neat.mutate_add_link_prob) {
					...
					if ((Math.random()> Neat.interspecies_mate_rate)) {
						//Mate within Species
						...
						while(randspecies == this && giveup < 5) {
							...
							java.util.Random randgaus = new java.util.Random();
							//Choose a random species tending towards better species
							randmult = randgaus.nextGaussian()/4; //gaussrand()/4;
							if (randmult > 1.0) 
								randmult = 1.0;
							//This tends to select better species
							randspeciesnum = Math.floorDiv((int)randmult, (int)((sorted_species.size()-1.0)+0.5)); //floor((randmult*(sorted_species.size()-1.0))+0.5);
							cursp = sorted_species.get(0);
							for(spcount = 0; spcount < randspeciesnum; spcount++)
								cursp = sorted_species.get(spcount); //.get(1);
							randspecies = cursp;
					...
					if (Math.random()< Neat.mate_multipoint_prob) { 
						new_genome = mom.gnome.mate_multipoint(dad.gnome, count, mom.orig_fitness, dad.orig_fitness, outside);
					}
					else if (Math.random() < (Neat.mate_multipoint_avg_prob/(Neat.mate_multipoint_avg_prob+Neat.mate_singlepoint_prob))) {
					...
					if ((Math.random()> Neat.mate_only_prob)||
				...
				curspecies = pop.species.get(0); //.begin();
				if (curspecies.equals(pop.species.lastElement())){
					//Create the first species
					newspecies = new Species(++(pop.last_species), true);
					pop.species.add(newspecies); //push_back(newspecies);
					newspecies.add_Organism(baby);  //Add the baby
					baby.species = newspecies;  //Point the baby to its species
				} 
				else {
					//comporg=(*curspecies)->first();
					comporg = curspecies.first(); //.firstElement();
					found = false;
					while(!curspecies.equals(pop.species.lastElement()) && !found) {	
							if (comporg.equals(0)) {
								//Keep searching for a matching species
								curspecies = pop.species.get(1); //curspecies.get(1);
								if (!curspecies.equals(pop.species.lastElement()))
									comporg = curspecies.first();
							}
							else if (baby.gnome.compatibility(comporg.gnome) < Neat.compat_thresh) {
								...
								//Keep searching for a matching species
								curspecies.equals(pop.species.get(1)); //get(1);
								if (!curspecies.equals(pop.species.lastElement())) 
									comporg = curspecies.first();
							}
						}
